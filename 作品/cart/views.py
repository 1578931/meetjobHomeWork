from django.shortcuts import render,redirect  #轉跳頁面

from django.http import HttpResponseRedirect #轉跳頁面
...
def addtocart(request,ctype=None,productid=None):
    ...
    if ctype=="add":  # 將商品加入至購物車中
        product = Goods.objects.get(id=productid)  # 會用get 是因為帶入的產品ID 一定有在資料表車
...
def cartordercheck(request):  # 訂單完成後，可以作訂單查詢用
    ...
    if orderid == '' and customemail == '':
        ...
        if order == None:
            ...
            details = models.DetailModel.objects.filter(dorder=order)  #filter不確定有無資料
